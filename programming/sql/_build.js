...
/* ============================================================
   CATEGORY SUFFIXES — matched at end of folder name
   ============================================================ */
const CATEGORY_SUFFIXES = [
  'interview-and-troubleshooting',
...
function parseFolder(name) {
  ...
  for (const cat of CATEGORY_SUFFIXES) {
    if (rest.endsWith('-' + cat)) {
      rest = rest.slice(0, -(cat.length + 1));
      category = cat;
      break;
    }
  }
  return { num, theme: rest, category };
}
...
const CATEGORIES = {
  'core-concepts': {
    ...
    subtitle: 'The essential ideas behind {THEME} — what it is, why it matters, and how it fits in modern SQL.',
    facts: [
      { big: 'SQL', lbl: 'Standard' },
      { big: 'ANSI', lbl: 'ISO/IEC 9075' },
      { big: 'Declarative', lbl: 'Paradigm' },
      { big: 'Set-Based', lbl: 'Logic' },
      { big: 'ACID', lbl: 'Transactions' }
    ],
    concepts: [
      { icon: '🎯', title: 'What It Is', desc: '{THEME} is a foundational SQL concept every database developer should master.' },
      { icon: '⚙️', title: 'How It Works', desc: 'The database engine parses, plans, and executes {THEME} at scale.' },
      { icon: '🚀', title: 'Why It Matters', desc: 'Understanding {THEME} unlocks performance and correctness.' },
      { icon: '🌍', title: 'Real-World Use', desc: 'Used in every app that stores data — banking, e-commerce, analytics.' }
    ],
    playground: {
      file: 'demo.sql',
      code: `-- {THEME} — core concepts demo
...
SELECT * FROM users ORDER BY age DESC;`
    ...
    steps: [
      { line: 0, text: '<code>CREATE TABLE</code> defines the schema — columns, types, constraints.', tags: ['DDL'] },
      { line: 1, text: '<code>PRIMARY KEY</code> — unique identifier for each row.', tags: ['Key'] },
      { line: 2, text: '<code>NOT NULL</code> — column must always have a value.', tags: ['Constraint'] },
      { line: 6, text: '<code>INSERT INTO</code> — adds rows. Column order must match values.', tags: ['DML'] },
      { line: 9, text: '<code>SELECT</code> — retrieves data. The most-used SQL statement.', tags: ['Query'] }
    ],
    recap: [
      { title: '{THEME}', text: 'Foundation of every database application.' },
      { title: 'Declarative', text: 'Describe *what* you want — SQL decides *how*.' },
      { title: 'Set-Based', text: 'Operates on sets of rows, not loops.' },
      { title: 'Standards', text: 'ANSI SQL + dialect extensions (Postgres, MySQL, SQL Server).' }
    ]
  },

  'practical-queries': {
    titleHtml: '{THEME}: <span class="grad">Practical Queries</span>',
    subtitle: 'Real, copy-paste SQL queries for {THEME} you can use in production today.',
    facts: [
      { big: 'Ready', lbl: 'Copy-Paste' },
      { big: 'Real', lbl: 'Use Case' },
      { big: 'Tested', lbl: 'Queries' },
      { big: 'Portable', lbl: 'Multiple DBs' },
      { big: 'Safe', lbl: 'Best Practice' }
    ],
    concepts: [
      { icon: '🎯', title: 'Real Scenario', desc: 'Common tasks developers actually perform with {THEME}.' },
      { icon: '🔧', title: 'Ready Queries', desc: 'Every example runs — no placeholder syntax.' },
      { icon: '🧩', title: 'Patterns', desc: 'Reusable shapes you can adapt to your schema.' },
      { icon: '⚡', title: 'Output', desc: 'What each query returns — verified.' }
    ],
    playground: {
      file: 'queries.sql',
      code: `-- {THEME} — practical query patterns
...
INSERT INTO orders (customer, amount, order_date) VALUES
    ('Alice', 120.50, '2024-01-15'),
    ('Bob', 89.99, '2024-01-16'),
    ('Alice', 250.00, '2024-01-17'),
    ('Carol', 45.00, '2024-01-18');
...
ORDER BY total DESC;`
    ...
    steps: [
      { line: 0, text: '<code>SELECT customer, SUM(amount)</code> — pick columns + aggregate.', tags: ['SELECT'] },
      { line: 2, text: '<code>GROUP BY</code> — collapse rows by a column. One row per customer.', tags: ['GROUP'] },
      { line: 3, text: '<code>ORDER BY ... DESC</code> — highest first.', tags: ['ORDER'] },
      { line: 4, text: '<code>LIMIT 10</code> — top 10 results. Use TOP in SQL Server.', tags: ['LIMIT'] }
    ],
    recap: [
      { title: 'Aggregate', text: '<code>SUM</code>, <code>COUNT</code>, <code>AVG</code>, <code>MAX</code>, <code>MIN</code>.' },
      { title: 'GROUP BY', text: 'Bucket rows before aggregating.' },
      { title: 'ORDER BY', text: 'Sort the final result set.' },
      { title: 'LIMIT', text: 'Restrict rows returned for performance.' }
    ]
  },

  'performance-and-design': {
    titleHtml: '{THEME}: <span class="grad">Performance &amp; Design</span>',
    subtitle: 'How to make {THEME} fast and how to design schemas that scale.',
    facts: [
      { big: 'EXPLAIN', lbl: 'Analyze' },
      { big: 'Index', lbl: 'Speed Up' },
      { big: 'Normalize', lbl: 'Design' },
      { big: 'Partition', lbl: 'Scale' },
      { big: 'Cache', lbl: 'Speed' }
    ],
    concepts: [
      { icon: '📊', title: 'Measure First', desc: 'Use <code>EXPLAIN</code> and query plans — never optimize blind.' },
      { icon: '🔥', title: 'Indexes', desc: 'The single biggest performance win. Index your JOIN and WHERE columns.' },
      { icon: '🧠', title: 'Schema Design', desc: 'Normalize to reduce duplication, denormalize for reads.' },
      { icon: '⚙️', title: 'Trade-offs', desc: 'Every index slows writes. Balance carefully.' }
    ],
    playground: {
      file: 'performance.sql',
      code: `-- {THEME} — performance patterns

-- Always analyze the query plan first
EXPLAIN SELECT * FROM users WHERE email = 'a@b.com';

-- Create an index on the lookup column
...
-- Analyze again — should show index usage
...
SELECT id, name FROM users WHERE age > 21;`
    ...
    stepCode: [
      ...
      '',
      '-- Add index for the lookup column',
      'CREATE INDEX idx_users_email ON users(email);',
      '',
    ...
    steps: [
      { line: 0, text: 'Comment explains the workflow — always measure before optimizing.', tags: ['Method'] },
      { line: 1, text: '<code>EXPLAIN</code> — shows the query plan. Look for full scans.', tags: ['EXPLAIN'] },
      { line: 4, text: '<code>CREATE INDEX</code> — the #1 performance tool. Speeds up lookups.', tags: ['Index'] },
      { line: 7, text: '<strong>Covering index</strong> — includes all columns the query needs.', tags: ['Covering'] }
    ],
    recap: [
      { title: 'EXPLAIN', text: 'Always check the plan. Full scans are the enemy.' },
      { title: 'Indexes', text: 'Add on columns in <code>WHERE</code>, <code>JOIN</code>, <code>ORDER BY</code>.' },
      { title: 'Avoid SELECT *', text: 'Fetch only what you need.' },
      { title: 'Test at Scale', text: 'What works on 1K rows may fail on 1M.' }
    ]
  },

  'interview-and-troubleshooting': {
    titleHtml: '{THEME}: <span class="grad">Interview &amp; Troubleshooting</span>',
    subtitle: 'Common interview questions and debugging patterns for {THEME}.',
    facts: [
      { big: 'FAQ', lbl: 'Common Qs' },
      { big: '10+', lbl: 'Interview' },
      { big: 'Real', lbl: 'Scenarios' },
      { big: 'Answers', lbl: 'Explained' },
      { big: 'Debug', lbl: 'Patterns' }
    ],
    concepts: [
      { icon: '🎤', title: 'Interview Qs', desc: 'The most common SQL interview questions about {THEME}.' },
      { icon: '💡', title: 'Best Answers', desc: 'Clear, correct explanations that impress interviewers.' },
      { icon: '🐛', title: 'Common Bugs', desc: 'Error messages you will actually see and how to fix them.' },
      { icon: '🎯', title: 'Follow-ups', desc: 'Where interviewers usually go next.' }
    ],
    playground: {
      file: 'interview.sql',
      code: `-- {THEME} — interview-style Q&A
...
-- Q: NULL handling — COUNT(*) vs COUNT(column)
SELECT
    COUNT(*)     AS total_rows,
...
WHERE salary < (SELECT MAX(salary) FROM employees);`
    ...
    steps: [
      { line: 1, text: 'Classic "find duplicates" — group + count.', tags: ['Q'] },
      { line: 3, text: '<code>HAVING</code> filters after grouping (WHERE runs before).', tags: ['HAVING'] },
      { line: 6, text: 'Second-highest — use a subquery with MAX.', tags: ['Subquery'] },
      { line: 7, text: 'Better: <code>LIMIT 1 OFFSET 1</code> with <code>ORDER BY DESC</code>.', tags: ['Alt'] }
    ],
    recap: [
      { title: 'HAVING vs WHERE', text: 'WHERE filters rows; HAVING filters groups.' },
      { title: 'NULL Logic', text: '<code>COUNT(*)</code> includes NULLs, <code>COUNT(col)</code> does not.' },
      { title: 'JOIN Types', text: 'INNER = match only. LEFT = all from left.' },
      { title: 'Practice', text: 'Explain each answer out loud before the interview.' }
    ]
  }
};
...
function titleFromSlug(slug) {
  const special = {
    sql: 'SQL', ddl: 'DDL', dml: 'DML', dcl: 'DCL', tcl: 'TCL',
    acid: 'ACID', join: 'JOIN', joins: 'JOINs', key: 'Key',
    index: 'Index', indexes: 'Indexes', null: 'NULL',
...
function buildTopic(folderName) {
  ...
  const tpl = CATEGORIES[category] || CATEGORIES['core-concepts'];
  ...
  return {
    num,
    title: sub('{THEME}').replace('{THEME}', themeTitle),
    titleHtml: sub(tpl.titleHtml),
    subtitle: sub(tpl.subtitle),
    facts: tpl.facts.map(f => ({ big: sub(f.big), lbl: sub(f.lbl) })),
    concepts: tpl.concepts.map(c => ({
      icon: c.icon,
      title: sub(c.title),
      desc: sub(c.desc)
    })),
    playground: {
      file: category === 'practical-queries' ? 'queries.sql' :
            category === 'performance-and-design' ? 'performance.sql' :
            category === 'interview-and-troubleshooting' ? 'interview.sql' : 'demo.sql',
...
function renderHTML(topic) {
  ...
  const conceptsHtml = topic.concepts.map((c, i) => {
    const colors = ['#00758f,#00b4d8', '#f59e0b,#fbbf24', '#8b5cf6,#a78bfa', '#22c55e,#4ade80'];
  ...
  const recapHtml = topic.recap.map((r, i) => {
    const colors = ['#00758f', '#f59e0b', '#8b5cf6', '#22c55e'];
...
.space-base{position:fixed;inset:0;z-index:0;background:radial-gradient(ellipse at 50% 0%,#0a1a26 0%,#0a0a20 45%,#05060f 85%),radial-gradient(ellipse at 100% 100%,#1a0f05 0%,transparent 55%)}
.orbs{position:fixed;inset:0;z-index:1;pointer-events:none;overflow:hidden;mix-blend-mode:screen;opacity:.45}
.orb{position:absolute;border-radius:50%;filter:blur(90px)}
.orb-1{width:520px;height:520px;top:-10%;left:-8%;background:radial-gradient(circle,#00758f,transparent 70%);animation:d1 26s ease-in-out infinite}
.orb-2{width:460px;height:460px;top:45%;right:-10%;background:radial-gradient(circle,#f59e0b,transparent 70%);animation:d2 30s ease-in-out infinite}
.orb-3{width:600px;height:600px;bottom:-18%;left:18%;background:radial-gradient(circle,#8b5cf6,transparent 70%);animation:d3 34s ease-in-out infinite}
...
.grid-bg{position:fixed;inset:-50%;z-index:2;pointer-events:none;opacity:.22;background-image:linear-gradient(rgba(0,117,143,.14) 1px,transparent 1px),linear-gradient(90deg,rgba(0,117,143,.14) 1px,transparent 1px);background-size:80px 80px;transform:perspective(500px) rotateX(60deg);animation:gf 22s linear infinite;mask-image:radial-gradient(ellipse at 50% 50%,black 20%,transparent 70%);-webkit-mask-image:radial-gradient(ellipse at 50% 50%,black 20%,transparent 70%)}
...
.brand-mark{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#00758f,#f59e0b);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:.85rem;color:#fff;box-shadow:0 8px 20px -6px rgba(0,117,143,.6)}
.hero{padding:4.5rem 1.5rem 3rem;text-align:center}
.tpill{display:inline-flex;align-items:center;gap:.5rem;background:rgba(0,117,143,.12);border:1px solid rgba(0,117,143,.35);border-radius:999px;padding:.45rem 1.1rem;font-size:.72rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:#66d4e8;margin-bottom:1.5rem}
.tpill .n{background:#00758f;color:#fff;min-width:24px;height:22px;border-radius:11px;display:inline-flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:900;padding:0 .4rem}
.hero h1{font-size:clamp(1.7rem,4.4vw,2.8rem);font-weight:900;letter-spacing:-.04em;line-height:1.15;margin-bottom:1.2rem}
.hero h1 .grad{background:linear-gradient(135deg,#66d4e8 0%,#00758f 45%,#f59e0b 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;background-size:200% 200%;animation:gs 7s ease-in-out infinite}
...
.stag{display:inline-flex;align-items:center;gap:.5rem;background:rgba(0,117,143,.1);border:1px solid rgba(0,117,143,.3);border-radius:999px;padding:.4rem 1rem;font-size:.68rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:#66d4e8;margin-bottom:1rem}
.sec-head h2{font-size:clamp(1.6rem,3.8vw,2.3rem);font-weight:900;letter-spacing:-.03em;line-height:1.15;margin-bottom:.6rem}
.sec-head h2 .grad{background:linear-gradient(135deg,#66d4e8,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.fact:hover{transform:translateY(-3px);border-color:rgba(0,117,143,.4)}
.fact .big{font-size:1.05rem;font-weight:900;background:linear-gradient(135deg,#66d4e8,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;line-height:1.2}
...
.editor-toolbar{display:flex;align-items:center;justify-content:space-between;padding:.6rem 1rem;background:#161b22;border-bottom:1px solid rgba(148,163,184,.1)}
...
.step-btn{background:rgba(0,117,143,.15);border:1px solid rgba(0,117,143,.3);color:#66d4e8;border-radius:8px;padding:.35rem .9rem;font-size:.75rem;font-weight:700;cursor:pointer;font-family:inherit;transition:all .2s}
.step-btn:hover:not(:disabled){background:rgba(0,117,143,.3)}
...
.step-line.active{background:rgba(0,117,143,.18);box-shadow:inset 3px 0 0 #00758f;color:#fff}
...
.tag{background:rgba(102,212,232,.15);border:1px solid rgba(102,212,232,.3);color:#66d4e8;border-radius:6px;padding:.25rem .65rem;font-size:.7rem;font-family:'JetBrains Mono',monospace;font-weight:600}
...
.bar-fill{height:100%;width:0%;background:linear-gradient(90deg,#00758f,#f59e0b);border-radius:999px;transition:width .35s}
...
.recap{background:rgba(30,41,59,.6);border-radius:14px;padding:1.1rem 1.2rem;border-left:3px solid var(--rc,#00758f)}
.recap .t{font-size:.72rem;font-weight:800;color:var(--rc,#00758f);text-transform:uppercase;letter-spacing:.08em}
...
<div class="page">
  <nav>
    <div class="nav-inner">
      <a href="#" class="brand"><div class="brand-mark">SQL</div> SQL Series</a>
    </div>
  </nav>
      ...
      <div class="playground reveal">
        <div class="editor-pane">
          <div class="editor-toolbar">
            ...
            <button class="run-btn" onclick="runCode()">▶ Run Query</button>
          </div>
          <textarea class="code-area" id="codeInput" spellcheck="false">${codeEscaped}</textarea>
        </div>
        <div class="output-pane">
          <div class="output-header"><span class="status-dot" id="statusDot"></span> RESULT</div>
          <div class="output-body" id="outputBody"><span class="dim">-- Click "Run Query" to see the result...</span></div>
        </div>
      </div>
    </section>
  ...
  <footer>
    <p>🗄️ ${escapeHtml(topic.title)} · SQL Series · Built for learners</p>
  </footer>
</div>
...
function simulateSQL(code){
  // Light SQL simulator — handles CREATE TABLE, INSERT, SELECT basics
  const tables = {};
  const output = [];
  let error = null;
  ...
  // Split on ;
  const stmts = joined.split(';').map(s => s.trim()).filter(s => s.length > 0);
  ...
  for(const stmt of stmts){
    // CREATE TABLE
    let cm = stmt.match(/^CREATE\\s+TABLE\\s+(\\w+)\\s*\\(([\\s\\S]+)\\)$/i);
    if(cm){
      const name = cm[1];
      const cols = cm[2].split(',').map(c => c.trim().split(/\\s+/)[0]);
      tables[name] = { cols, rows: [] };
      output.push('✓ Created table ' + name + ' (' + cols.join(', ') + ')');
      continue;
    }

    // INSERT INTO
    let im = stmt.match(/^INSERT\\s+INTO\\s+(\\w+)\\s*(?:\\([^)]+\\))?\\s*VALUES\\s*([\\s\\S]+)$/i);
    if(im){
      const name = im[1];
      const rows = [];
      const valuesPart = im[2];
      // Match each (...) block
      const rx = /\\(([^()]*)\\)/g;
      let m;
      while((m = rx.exec(valuesPart)) !== null){
        const vals = m[1].split(',').map(v => parseValue(v));
        rows.push(vals);
      }
      if(tables[name]){
        rows.forEach(r => tables[name].rows.push(r));
        output.push('✓ Inserted ' + rows.length + ' row(s) into ' + name);
      }
      continue;
    }

    // SELECT
    let sm = stmt.match(/^SELECT\\s+([\\s\\S]+?)\\s+FROM\\s+(\\w+)([\\s\\S]*)$/i);
    if(sm){
      const selectPart = sm[1].trim();
      const tableName = sm[2];
      const rest = (sm[3] || '').trim();
      const table = tables[tableName];
      if(!table){
        output.push('✗ Table not found: ' + tableName);
        continue;
      }

      // Parse ORDER BY
      let orderBy = null, orderDir = 'ASC', limit = null;
      let restClean = rest;
      let om = restClean.match(/ORDER\\s+BY\\s+([\\w.]+)(?:\\s+(ASC|DESC))?/i);
      if(om){
        orderBy = om[1].replace(/\\w+\\./, '');
        orderDir = (om[2] || 'ASC').toUpperCase();
      }
      let lm = restClean.match(/LIMIT\\s+(\\d+)/i);
      if(lm) limit = parseInt(lm[1], 10);

      // Aggregate handling
      let aggMatch = selectPart.match(/^(COUNT|SUM|AVG|MAX|MIN)\\s*\\(\\s*(\\*|[\\w.]+)\\s*\\)(?:\\s+AS\\s+(\\w+))?$/i);
      let groupBy = null;
      let gm = restClean.match(/GROUP\\s+BY\\s+([\\w.]+)/i);
      if(gm) groupBy = gm[1].replace(/\\w+\\./, '');

      let cols = selectPart.split(',').map(c => c.trim());
      let rows = table.rows.slice();

      if(groupBy){
        const groups = {};
        const gidx = table.cols.indexOf(groupBy);
        rows.forEach(r => {
          const key = r[gidx];
        ...
        for(const k in groups){
          ...
          const agg = aggMatch ? aggMatch[1].toUpperCase() : 'COUNT';
          const colRef = aggMatch && aggMatch[2] !== '*' ? aggMatch[2].replace(/\\w+\\./, '') : null;
          const colIdx = colRef ? table.cols.indexOf(colRef) : -1;
          let val;
          if(agg === 'COUNT') val = group.length;
          else if(agg === 'SUM') val = group.reduce((a,r) => a + (Number(r[colIdx])||0), 0);
          else if(agg === 'AVG') val = group.reduce((a,r) => a + (Number(r[colIdx])||0), 0) / group.length;
          else if(agg === 'MAX') val = Math.max(...group.map(r => Number(r[colIdx])||0));
          else if(agg === 'MIN') val = Math.min(...group.map(r => Number(r[colIdx])||0));
          resultRows.push([k, val]);
        }
        // Sort
        if(orderBy){
          const idx = orderBy === groupBy ? 0 : 1;
          resultRows.sort((a,b) => {
            if(a[idx] < b[idx]) return orderDir === 'ASC' ? -1 : 1;
            if(a[idx] > b[idx]) return orderDir === 'ASC' ? 1 : -1;
            return 0;
          });
        }
        const headers = [groupBy, (aggMatch?.[3] || aggMatch?.[1] || 'COUNT')];
        let tbl = formatTable(headers, resultRows);
        output.push(tbl);
        continue;
      }
      ...
      // Non-aggregate
      if(orderBy){
        const idx = table.cols.indexOf(orderBy);
        if(idx >= 0){
          rows.sort((a,b) => {
            if(a[idx] < b[idx]) return orderDir === 'ASC' ? -1 : 1;
            if(a[idx] > b[idx]) return orderDir === 'ASC' ? 1 : -1;
      ...
      // Projection
      let headers, resultRows;
      if(cols.length === 1 && cols[0] === '*'){
        headers = table.cols;
        resultRows = rows;
      } else {
        const idxs = cols.map(c => {
          const clean = c.replace(/\\s+AS\\s+\\w+$/i, '').trim().replace(/\\w+\\./, '');
          return table.cols.indexOf(clean);
        });
        headers = cols.map(c => {
          const asMatch = c.match(/\\s+AS\\s+(\\w+)$/i);
          if(asMatch) return asMatch[1];
          return c.replace(/\\w+\\./, '').trim();
        });
        resultRows = rows.map(r => idxs.map(i => i >= 0 ? r[i] : null));
      }

      output.push(formatTable(headers, resultRows));
      continue;
    }

    // ALTER, CREATE INDEX, DROP — acknowledge
    let other = stmt.match(/^(ALTER|CREATE\\s+INDEX|DROP|TRUNCATE|UPDATE|DELETE)\\s+/i);
    if(other){
      output.push('✓ Executed: ' + stmt.slice(0, 60) + (stmt.length > 60 ? '...' : ''));
      continue;
    }

    // EXPLAIN — acknowledge
    if(/^EXPLAIN/i.test(stmt)){
      output.push('✓ Query plan generated (simulated)');
      continue;
    }
  }
  ...
  function formatTable(headers, rows){
    const widths = headers.map((h, i) =>
      Math.max(String(h).length, ...rows.map(r => String(r[i] ?? 'NULL').length))
    );
    const sep = '+' + widths.map(w => '-'.repeat(w + 2)).join('+') + '+';
    const head = '|' + headers.map((h, i) => ' ' + String(h).padEnd(widths[i]) + ' ').join('|') + '|';
    const body = rows.map(r =>
      '|' + r.map((c, i) => ' ' + String(c ?? 'NULL').padEnd(widths[i]) + ' ').join('|') + '|'
    ).join('\\n');
    return sep + '\\n' + head + '\\n' + sep + '\\n' + body + '\\n' + sep;
  }
}
...
function runCode(){
  ...
  try {
    const r = simulateSQL(code);
    ob.innerHTML = '';
    sd.className = 'status-dot ok';
    if(r.output.length === 0){
      ob.innerHTML = '<span class="dim">-- No statements executed.</span>';
    } else {
    ...
    ob.innerHTML = '<span class="err">✗ Error: ' + e.message + '</span>';
  }
}
...
console.log(`\n🗄️  Found ${folders.length} SQL topic folders.\n`);
